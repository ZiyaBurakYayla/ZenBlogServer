...
{
    public class GetLoginQueryHandler(UserManager<AppUser> _userManager,IJwtService _jwtService,IMapper _mapper) : 
        IRequestHandler<GetLoginQuery, BaseResult<GetLoginQueryResult>>
    {
        public async Task<BaseResult<GetLoginQueryResult>> Handle(GetLoginQuery request, CancellationToken cancellationToken)
        {
            ...
            if(!result)
            {
                return BaseResult<GetLoginQueryResult>.Fail("Email or Password in incorret"); 
            }

            var userResult = _mapper.Map<GetUserQueryResult>(user);
